...
#0000 if `docker run` first argument start with `-` the user is passing jenkins swarm launcher arguments
if [[ $# -lt 1 ]] || [[ "$1" == "-"* ]]; then

  # jenkins swarm slave
  JAR=`ls -1 /usr/share/jenkins/swarm-client-*.jar | tail -n 1`
  exec java $JAVA_OPTS -jar $JAR -fsroot $HOME -url "http://$JENKINS_PORT_8080_TCP_ADDR:$JENKINS_PORT_8080_TCP_PORT" -username $USERNAME -password $PASSWORD -labels $LABELS -executors $EXECUTORS
